...
> 本文档详细介绍了 Nexus Agent 项目的架构、核心模块和代码实现，帮助你快速理解和学习项目代码。涵盖了 Sprint 1-6 的完整实现，包括对话代理、RAG 系统、工具调用、记忆管理、API 服务和前端开发。
...
5. [工具调用模块详解 - Sprint 3](#5-工具调用模块详解---sprint-3)
6. [记忆管理模块详解 - Sprint 4](#6-记忆管理模块详解---sprint-4)
7. [API 服务模块详解 - Sprint 5](#7-api-服务模块详解---sprint-5)
8. [前端开发模块详解 - Sprint 6](#8-前端开发模块详解---sprint-6)
9. [完整数据流图](#9-完整数据流图)
10. [使用示例](#10-使用示例)
11. [学习路径建议](#11-学习路径建议)
12. [关键设计模式](#12-关键设计模式)
...
**Nexus Agent** 是一个基于 LangChain 1.0 的智能对话系统，专为帮助新员工快速适应工作环境而设计。项目采用模块化架构，结合了对话代理、检索增强生成（RAG）、工具调用、记忆管理、API 服务和前端开发能力。
...
| 💬 多轮对话 | 支持上下文感知的多轮对话 | 1, 2, 4 |
| 🛠️ 工具调用 | 内置公司政策查询、联系方式获取等工具 | 1, 3 |
| 📚 知识检索 | 从公司文档中检索相关信息，提供准确答案 | 2 |
| 🔍 语义搜索 | 使用向量嵌入进行语义相似度搜索 | 2 |
| 📊 监控追踪 | 完整的日志记录和 Token 使用追踪 | 1, 5 |
| 🧪 测试完善 | 包含全面的安全测试和对话流程测试 | 1, 2, 3 |
| 🔧 易于配置 | 通过环境变量灵活配置 | 1, 2, 4 |
| ⚡ 中间件架构 | 使用 LangChain 1.0 中间件模式，模块化可扩展 | 1 |
| 🇨🇳 中文优化 | 使用 BGE 中文嵌入模型，优化中文语义理解 | 2 |
| 💾 会话持久化 | 基于 Redis 的对话历史持久化 | 4 |
| 🌐 RESTful API | OpenAI 兼容的 API 接口 | 5 |
| 🎨 现代前端 | React 18 + Vite 前端应用 | 6 |
| 📱 响应式设计 | 支持多设备访问 | 6 |
...
**Sprint 3 - 工具调用：**
- **工具定义**: LangChain 1.0 `@tool` 装饰器
- **工具绑定**: `bind_tools()` 方法绑定工具到模型
- **参数提取**: 自动从用户输入中提取工具参数
- **元数据追踪**: 记录工具调用次数、执行时间等

**Sprint 4 - 记忆管理：**
- **Redis 客户端**: redis-py 客户端封装
- **会话管理**: SessionManager 类管理用户会话
- **上下文压缩**: ContextManager 类压缩对话历史
- **Token 管理**: 智能管理对话上下文长度
...
│   ├── demo_tool_calling.py        # Sprint 3 工具调用演示
│   ├── demo_memory_management.py    # Sprint 4 记忆管理演示
│   └── README.md                   # 演示脚本说明文档
├── scripts/                    # 启动脚本目录
...
### 第五阶段：实践运行
1. 运行 [`demos/demo_script.py`](demos/demo_script.py) - 观察 Sprint 1 实际运行效果
2. 运行 [`demos/demo_rag.py`](demos/demo_rag.py) - 观察 Sprint 2 RAG 功能
3. 运行 [`demos/demo_document_processing.py`](demos/demo_document_processing.py) - 观察文档处理流程
4. 运行测试套件 - 理解测试用例
5. 尝试修改配置 - 观察不同配置的效果
...
| [`demos/demo_script.py`](demos/demo_script.py) | Sprint 1 演示脚本 | 1 | ⭐⭐ |
| [`demos/demo_rag.py`](demos/demo_rag.py) | Sprint 2 RAG 演示 | 2 | ⭐⭐ |
| [`demos/demo_document_processing.py`](demos/demo_document_processing.py) | 文档处理演示 | 2 | ⭐⭐ |
